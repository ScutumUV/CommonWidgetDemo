...
public class SwipeItemLayout extends ViewGroup {
    ...
    private void readAttrs(Context context, AttributeSet attrs) {
        ...
        try {
            contentId = typedArray.getResourceId(R.styleable.SwipeItemLayout_mainMenuId, 0);
            rightMenuId = typedArray.getResourceId(R.styleable.SwipeItemLayout_rightMenuId, 0);
        } finally {
            typedArray.recycle();
        }
    }
    ...
    private Mode getTouchMode() {
        return mTouchMode;
    }

    private void setTouchMode(Mode mode) {
        switch (mTouchMode) {
            case FLING:
    ...
    private boolean trackMotionScroll(int deltaX) {
        if (deltaX == 0)
            return false;
    ...
    private boolean ensureChildren() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (childView.getId() == contentId) {
                mMainView = childView;
            } else if (childView.getId() == rightMenuId) {
                mSideView = childView;
            } else {
                return false;
            }
    ...
    private void offsetChildrenLeftAndRight(int delta) {
    ...
    private class ScrollRunnable implements Runnable {
        ...
        private void startScroll(int startX, int endX) {
            if (startX != endX) {
        ...
        private void startFling(int startX, int xVel) {
...
//        return null;
        ...
        for (int i = childCount - 1; i >= 0; i--) {
            ...
            if (x >= child.getLeft() && x < child.getRight()
                    && y >= child.getTop() && y < child.getBottom()) {
                if (!(child instanceof SwipeItemLayout) && child instanceof ViewGroup) {
                    for (int j = 0; j < ((ViewGroup) child).getChildCount(); j++) {
                        returnV = ((ViewGroup) child).getChildAt(j);
                        if (null != returnV && returnV instanceof SwipeItemLayout) {
                            return returnV;
                        }
                    }
                } else if (child instanceof SwipeItemLayout) {
                    returnV = child;
                    return returnV;
                }
            }
